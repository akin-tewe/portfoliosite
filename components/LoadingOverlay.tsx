...
export default function PageLoader({ loading }: { loading: boolean }) {
    return (
        <AnimatePresence>
            {loading && (
                <motion.div
                    className="fixed inset-0 z-[500] flex items-center justify-center bg-gray-100"
                    initial={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
...
export function BootOverlay() {
    ...
    return (
        <AnimatePresence>
            {show && (
                <motion.div
                    className="fixed inset-0 z-[500] flex items-center justify-center pointer-events-none bg-gray-100"
                    initial={{ opacity: 1 }}
