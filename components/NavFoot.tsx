...
export default function NavBar() {
    ...
    return(
        <header className="flex justify-between items-center px-8 py-4">
            <Link href="/" className="text-white/30 hover:text-white/100 transition-all z-20" onClick={() => {show(); setTimeout(hide,800)}}><House/></Link>
            <nav className={`flex gap-6 z-10 ${pixelify.className}`}>
...
export function Footer() {
    ...
    return(
          <footer  className="flex justify-between p-10"> {/* Footer Element */}
            <span className={`text-[#1650a7] opacity-70  text-xl ${pixelify.className}`}>©coded by akin tewe</span>
