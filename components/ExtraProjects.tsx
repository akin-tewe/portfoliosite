...
export default function ExtraProjects() {
    ...
    return(
        <div className="flex flex-col p-10 md:px-30 mt-10 w-full mx-auto ">
            ...
            <div key={proj.id}>
                <Link href={proj.link} onClick={() => {show(); setTimeout(hide,800)}}>
                    <button className="group py-12 flex text-white items-center justify-between w-full px-8">
                        ...
                        <span className={`${roboto.className} font-light group-hover:translate-x-7 transition-all group-hover:opacity-60 hidden md:block`}>{proj.desc}</span>
                    </button>
                </Link>
                <div className="h-px bg-white/30"/>
            </div>
